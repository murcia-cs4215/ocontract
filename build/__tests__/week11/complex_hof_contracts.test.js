...
f g;;`);
    ...
    (0, tests_1.expectContractViolation)(res, 'main', 4, 0);
});
...
f g;;`);
    ...
    (0, tests_1.expectContractViolation)(res, 'g', 4, 0);
});
...
test("function with hof contract that returns a result violating this function's return value contract", () => {
    ...
    const res = (0, tests_1.runTest)(`contract f = ({x : int | x > 0} -> {y : int | true}) -> {z : int | z > 0};;
let f (a : int -> int) : int  = a 10;;
...
test('hof passed twice with violation on original contract', () => {
    ...
    let res = (0, tests_1.runTest)(`contract f = ({x : int | x > 0} -> {y : int | y > 0}) -> {z : int | z > 0};;
let f (a : int -> int) : int  = a 10;;
...
h g;;`);
    ...
    (0, tests_1.expectContractViolation)(res, 'main', 7, 0);
    // Violate original return value contract
    res =
        (0, tests_1.runTest)(`contract f = ({x : int | x > 0} -> {y : int | y > 0}) -> {z : int | z > 0};;
let f (a : int -> int) : int  = a 10;;
...
h g;;`);
    // Blame g for violating its own contract
    (0, tests_1.expectContractViolation)(res, 'g', 7, 0);
});
...
h g;;`);
    ...
    (0, tests_1.expectContractViolation)(res, 'h', 4, 0);
...
h g;;`);
    ...
    (0, tests_1.expectContractViolation)(res, 'h', 1, 0);
});
//# sourceMappingURL=complex_hof_contracts.test.js.map