...
An OContract program is essentially a sequence of OContract statements. In order to have a rigorous definition of OContract statements,
we will first define the set primitive values and symbols, followed by expressions and contract declaration statements.
...
\hfill
$\Rule{E \qquad E_1 \qquad E_2}{\texttt{if } E\ \texttt{ then }\ E_1\ \texttt{ else } E_2}$
\hfill
$\Rule{E_1 \qquad E_2}{\texttt{let } x\ \texttt{ = }\ E_1\ \texttt{ in } E_2}$

$\Rule{E\quad E_1}{E\ E_1}$ [Function Application]
\hspace{2cm}
$\Rule{E}{\texttt{fun } (x_1 : t_1) \texttt{ } \cdots \texttt{ } (x_n : t_n) : t_{n+1} \rightarrow E}$

\subsubsection{Contracts Declaration}
