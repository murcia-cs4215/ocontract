...
test('lambda currying with call', () => {
  const res =
    runTest(`let x : int -> int -> int = fun (a : int) (b : int) : int -> a + b;;
let y : int -> int = x 10;;
y 20;;`);
  expect(res).toEqual({
    status: 'finished',
...
test('lambda declaration without assigning it to a variable', () => {
  const res = runTest('(fun (x : int) : int -> x + 1) 2;;');
  expect(res).toEqual({
    status: 'finished',
    value: 3,
    type: intType,
...
test('passing in lambda as argument to hof', () => {
  const res = runTest(`let f (g : int -> int) : int = g 2;;
f (fun (x : int) : int -> x);;`);
  expect(res).toEqual({
    status: 'finished',
    value: 2,
...
test('returning lambda in hof', () => {
  const res =
    runTest(`let f (g : int -> int) : int -> int = fun (x : int) : int -> x + g 2;;
f (fun (x : int) : int -> x) 2;;`);
  expect(res).toEqual({
    status: 'finished',
    value: 4,
...
test('variable that is defined as lambda', () => {
  const res = runTest(`let f : int -> int = fun (x : int) : int -> x + 1;;
f 1;;`);
  expect(res).toEqual({
    status: 'finished',
    value: 2,
...
test('recursive function that is a variable that is defined as lambda', () => {
  const res =
    runTest(`let rec f : int -> int = fun (x : int) : int -> if x == 0 then 1 else f (x - 1) * x;;
f 5;;`);
  expect(res).toEqual({
    status: 'finished',
    value: 120,
